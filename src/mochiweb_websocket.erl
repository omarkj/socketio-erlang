...
create_ws(Req, Version, Autoexit) ->
	Ws = [
		{version, Version}, {socket, Req:get(socket)}, {scheme, Req:get(scheme)},
		{path, Req:get(path)}, {headers, Req:get(headers)}, {autoexit, Autoexit}
	],% This should be a record, or I could change this to whatever Mochi uses by default
		
	Headers = proplists:get_value(headers, Ws),
	Host = mochiweb_headers:get_value(host, Headers),
	Origin = mochiweb_headers:get_value(origin, Headers),
	Socket = proplists:get_value(socket, Ws),
	...
	io:format("Handshake is ~p~n", [Handshake]),
	
	% Send the handshake
	mochiweb_socket:send(Socket, Handshake),
	io:format("SO, WS? ~p~n", Ws),
	void.
